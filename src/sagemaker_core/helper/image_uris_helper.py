...
from sagemaker_core.helper.session_helper import Session, botocore_resolver
from sagemaker_core.helper.version import Version
...
GRAVITON_ALLOWED_FRAMEWORKS = {"tensorflow", "pytorch", "xgboost", "sklearn"}
...
def retrieve(
    ...
    endpoint_data = botocore_resolver().construct_endpoint("ecr", region)
...
def _validate_py_version_and_set_if_needed(py_version, version_config, framework):
    ...
    if py_version is None and SPARK_NAME == framework:
        return None
