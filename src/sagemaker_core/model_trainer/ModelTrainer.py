...
from sagemaker_core.model_trainer.utils import (
    _is_valid_ecr_image, 
    _is_valid_s3_uri, 
    _is_valid_path, 
    _get_unique_name, 
...
class ContainerScriptConfig(Base):
    """
...
class ModelTrainer:
    ...
    def _default_stopping_condition() -> StoppingCondition:
        ...
        logger.info(f"Stopping Condition was not provided. Using default: {x.__dict__}")
        return x
    ...
    def _default_output_data_config(session: Session, base_name: str) -> OutputDataConfig:
        ...
        logger.info(f"Output Data Config was not provided. Using default: {x.__dict__}")
        return x

    def __init__(
        self,
        training_image: Optional[Union[str | FrameworkImageSpec]] = None,
        training_input_mode: Optional[str] = None,
        algorithm_name: Optional[str] = None,
        container_script_config: Optional[ContainerScriptConfig] = None,
        ...
        elif isinstance(training_image, FrameworkImageSpec):
        ...
        self.distribution = distribution
        self.container_script_config = container_script_config
    ...
    def run(
        ...
        Args:
            ...
            container_script_config (Optional[ContainerScriptConfig]):
        ...
        container_script_config = container_script_config or self.container_script_config
        if container_script_config:
            source_code_input_channel = self._get_container_source_code_channel(container_script_config)
            commands = container_script_config.command.split(" ")
            container_entrypoint = commands[0]
            container_entrypoint = [container_entrypoint]
            container_arguments = commands[1:]
    ...
    def _get_container_source_code_channel(self, container_script_config: ContainerScriptConfig) -> Channel:
        source_code_input_channel = None
        if isinstance(container_script_config.source_dir, S3DataSource):
            source_code_input_channel = Channel(
                channel_name="code",
                data_source=DataSource(
                    s3_data_source=container_script_config.source_dir
                ),
                input_mode="File"
            )
        if isinstance(container_script_config.source_dir, str) and  _is_valid_s3_uri(container_script_config.source_dir, path_type="Directory"):
            source_code_input_channel = Channel(
                channel_name="code",
                data_source=DataSource(
                    s3_data_source=S3DataSource(
                        s3_data_type="S3Prefix",
                        s3_uri=container_script_config.source_dir,
                        s3_data_distribution_type="FullyReplicated"
                    )
                ),
                input_mode="File"
            )
        elif isinstance(container_script_config.source_dir, str) and _is_valid_path(container_script_config.source_dir, path_type="Directory"):
            
            source_code_s3_uri = self.session.upload_data(
                path=container_script_config.source_dir,
                bucket=self.session.default_bucket(),
            ...
            if not _is_valid_s3_uri(container_script_config.source_dir, path_type="Directory"):
                raise ValueError(f"Invalid S3 directory uri path: {container_script_config.source_dir}")
            if not _is_valid_path(container_script_config.source_dir, path_type="Directory"):
                raise ValueError(f"Invalid local directory path: {container_script_config.source_dir}")

            raise ValueError(f"Invalid source_dir: {container_script_config.source_dir}")
