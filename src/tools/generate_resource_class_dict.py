...
class ResourceExtractor:
    ...
    def _extract_dataframes(self):
        ...
        for resource, actions in self.resource_actions.items():
            class_methods = set()
            object_methods = set()
            ...
            for action in actions:
                ...
                if action.split(resource)[0] == 'describe':
                    class_methods.add('get')
                    object_methods.add('refresh')
                    continue

                if action.split(resource)[0] in self.CLASS_METHODS:
                    class_methods.add(action.split(resource)[0])
                elif action.split(resource)[0] in self.OBJECT_METHODS:
                    object_methods.add(action.split(resource)[0])
                else:
        ...
        df.to_csv('resource_actions.csv', index=False)
