...
class ResourceGenerator(Generator):
    ...
    def generate_wait_method(self, resource) -> str:
        """Auto-Generate WAIT Method for a waitable resource.

        Usage:
        wait_method_response = res_gen.generate_wait_method("Model")
        from pprint import pprint
        pprint(get_method_response)
        
        :param resource: (str) Resource Name string. (Ex. Model)
        :return: (str) Formatted Wait Method template.
        """
        resource_operation = self.operations["Describe" + resource]
        resource_operation_input_shape_name = resource_operation["input"]["shape"]
        resource_operation_output_shape_name = resource_operation["output"]["shape"]
        
        operation_output_member_data = self.shapes[resource_operation_output_shape_name]["members"]
        
        if f"{resource}Status" in operation_output_member_data:
            status_shape_name =  f"{resource}Status"
        elif "Status" in operation_output_member_data:
            status_shape_name = "Status"
        else:
            return None
        ...
        formatted_method = WAIT_METHOD_TEMPLATE.format(
            operation_input_args=operation_input_args,
            operation=operation,
            status_shape_name=status_shape_name
        )
        return formatted_method
